...
#
# Script description: Run mixed-effect iSSF model in glmmTMB for fall season
#
...
system.time(
  ...
  m <- glmmTMB::glmmTMB(
    ...
    case_ ~ -1 +
      ...
      tri_end + I(tri_end^2) +
      northness_end + mine_end + elev_end + 
      ...
      # crossing variables
      crossing_lake_reservoir + crossing_hwy + crossing_town + crossing_rock_ice + 
      
      # GFR interactions
      greenness_end:greenness_broad_end + 
      ...
      greenness_end:sex_01 + canopy_cover_end:sex_01 + I(canopy_cover_end^2):sex_01 + tri_end:sex_01 + I(tri_end^2):sex_01 +
      northness_end:sex_01 + mine_end:sex_01 + elev_end:sex_01 + 
                        
      # habitat interactions and sex
      greenness_end:canopy_cover_end:sex_01 +
      
      # distance to spring starting location and sex
      d_spring_start_end:sex_01 +
      
      # crossing variables and sex
      crossing_lake_reservoir:sex_01 + 
      crossing_hwy:sex_01 + 
      crossing_town:sex_01 + 
      crossing_rock_ice:sex_01 + 
                        
      # GFR interactions and sex
      greenness_end:greenness_broad_end:sex_01 + 
      ...
      (0 + canopy_cover_end | id) +
      (0 + I(canopy_cover_end^2) | id) +
      (0 + greenness_end |id) + 
      (0 + tri_end |id) +
      (0 + I(tri_end^2) | id) + 
      (0 + northness_end | id)  +
    ...
    # (the first random effect), but to do so for the remaining 20 random slopes.
    ...
    data = dplyr::filter(df_issf_scaled, season == "fall"), family = poisson,
