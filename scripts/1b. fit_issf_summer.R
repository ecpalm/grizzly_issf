...
#
# Script description: Run mixed-effect iSSF model in glmmTMB for summer season
#
...
system.time(
  ...
  m <- glmmTMB::glmmTMB(
    ...
    case_ ~ -1 +
      ...
      tri_end + I(tri_end^2) + elev_end + northness_end +
      mine_end + d_hwy_decay_end + d_road_decay_end +
      ...
      # GFR interactions
      canopy_cover_end:greenness_broad_end + 
      d_hwy_decay_end:greenness_broad_end + d_road_decay_end:greenness_broad_end +  
      crossing_hwy:greenness_broad_end + crossing_rock_ice:greenness_broad_end +  
      ...
      greenness_end:sex_01 + 
      canopy_cover_end:sex_01 + I(canopy_cover_end^2):sex_01 +
      tri_end:sex_01 + I(tri_end^2):sex_01 + 
      elev_end:sex_01 + northness_end:sex_01 +
      mine_end:sex_01 + d_hwy_decay_end:sex_01 + d_road_decay_end:sex_01 +
      
      # habitat interactions and sex
      greenness_end:elev_end:sex_01 + 
      greenness_end:canopy_cover_end:sex_01 +
      d_spring_start_end:sex_01 +
                          
      # crossing variables and sex
      crossing_hwy:sex_01 + crossing_lake_reservoir:sex_01 + 
      crossing_town:sex_01 + crossing_rock_ice:sex_01 + 
      
      # GFR interactions and sex                    
      d_hwy_decay_end:greenness_broad_end:sex_01 + 
      d_road_decay_end:greenness_broad_end:sex_01 +  
      crossing_hwy:greenness_broad_end:sex_01 + 
      crossing_rock_ice:greenness_broad_end:sex_01 +  
      canopy_cover_end:greenness_broad_end:sex_01 +
      
      # movement variables and sex                    
      cos_ta:sex_01 + log_sl_km:sex_01 + 
                          
      # movement interactions and sex
      (canopy_cover_start + tri_start + greenness_start + elev_start):log_sl_km:sex_01 +
      ...
      (0 + d_hwy_decay_end | id) + (0 + d_road_decay_end | id) +
      (0 + elev_end | id) + (0 + northness_end | id) +
    ...
    # (the first random effect), but to do so for the remaining 21 random slopes.
    ...
    map=list(theta=factor(c(NA, 1:21))), start=list(theta=c(log(1e3), rep(0, 21))),
